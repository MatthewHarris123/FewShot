...
FLOWER_SPECS_DIR = Path("data/Flowers")


class Flowers(EasySet):
    def __init__(self, split: str, **kwargs):
        ...
        """
        specs_file = FLOWER_SPECS_DIR / f"{split}.json"
        if not specs_file.is_file():
            raise ValueError(
                f"Could not find specs file {specs_file.name} in {FLOWER_SPECS_DIR}"
            )
