...
FLOWER5_SPECS_DIR = Path("data/Flowers5")


class Flowers5(EasySet):
    def __init__(self, split: str, **kwargs):
        ...
        """
        specs_file = FLOWER5_SPECS_DIR / f"{split}.json"
        if not specs_file.is_file():
            raise ValueError(
                f"Could not find specs file {specs_file.name} in {FLOWER5_SPECS_DIR}"
            )
