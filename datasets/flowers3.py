...
FLOWER3_SPECS_DIR = Path("data/Flowers3")


class Flowers3(EasySet):
    def __init__(self, split: str, **kwargs):
        ...
        """
        specs_file = FLOWER3_SPECS_DIR / f"{split}.json"
        if not specs_file.is_file():
            raise ValueError(
                f"Could not find specs file {specs_file.name} in {FLOWER3_SPECS_DIR}"
            )
