...
FLOWER2_SPECS_DIR = Path("data/Flowers2")


class Flowers2(EasySet):
    def __init__(self, split: str, **kwargs):
        ...
        """
        specs_file = FLOWER2_SPECS_DIR / f"{split}.json"
        if not specs_file.is_file():
            raise ValueError(
                f"Could not find specs file {specs_file.name} in {FLOWER2_SPECS_DIR}"
            )
